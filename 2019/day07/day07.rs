...
impl IntCodeVM {
    ...
    fn run_to_next_output<F>(&mut self, mut next_input: F) -> Option<i32>
    where
        F: FnMut() -> i32,
    {
        loop {
            let status = self.step(&mut next_input);

            match status {
                VMStatus::Halted => return None,
                VMStatus::Output(x) => return Some(x),
                VMStatus::Continue => (),
            }
        }
    }

    fn run_until_input_consumed(&mut self, input: i32) -> Vec<i32> {
        let mut outputs = Vec::new();
        let mut consumed_input = false;

        while !consumed_input {
            let status = self.step(|| {
                consumed_input = true;
                input
            });
...
fn permutations<T>(mut vec: Vec<T>) -> Vec<Vec<T>>
where
    T: Clone,
{
    let mut result = Vec::new();
    perm_recurse(vec.len(), &mut vec, &mut result);
    result
}

fn perm_recurse<T>(n: usize, vec: &mut Vec<T>, out: &mut Vec<Vec<T>>)
where
    T: Clone,
{
    if n <= 1 {
...
fn run_amplifiers(program: &Vec<i32>, settings: &Vec<i32>) -> i32 {
    ...
    for i in settings {
        ...
        vm.run_until_input_consumed(*i);
        last_output = vm.run_to_next_output(|| last_output).unwrap();
    }
    last_output
}

fn run_amplifiers_loop(program: &Vec<i32>, settings: &Vec<i32>) -> i32 {
    let mut last_output = 0;
    let mut vms = Vec::new();
    for i in settings {
        let mut vm = IntCodeVM::new(program.clone());
    ...
    loop {
        for mut vm in &mut vms {
            match vm.run_to_next_output(|| last_output) {
                None => return last_output,
                Some(x) => last_output = x,
            }
        }
    }
}
...
fn star1(program: &Vec<i32>) -> i32 {
    let mut maximum = 0;
    let perms = permutations((0..5).collect());
    for i in perms {
        let output = run_amplifiers(program, &i);
        if output > maximum {
            maximum = output;
        }
    }
...
fn star2(program: &Vec<i32>) -> i32 {
    let mut maximum = 0;
    let perms = permutations((5..10).collect());
    for i in perms {
        let output = run_amplifiers_loop(program, &i);
        if output > maximum {
            maximum = output;
        }
    }
